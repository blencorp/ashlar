...
- ~~**Validator wedge**~~ **Shipped 2026-04-29** (slice 2). ast-grep via @ast-grep/napi; CEM-to-rule compiler; `audit --policy components`, `--policy federal`, and `--policy all`; per-language support matrix returns `language-unsupported` for Twig/Jinja/Nunjucks; SARIF carries line/column; standalone audit works without config or lockfile; published `ashlar-cem.schema.json` and `config.schema.json` validated at runtime via Ajv. *Spec*: [validator-wedge-spec](docs/superpowers/specs/2026-04-29-v0-0-validator-wedge-spec.md).
