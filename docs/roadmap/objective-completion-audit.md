...
Current testing evidence: `pnpm testing:start --check --visual` passed on 2026-05-08 after rebuilding the workspace. It verified HTTP 200 for the public site, docs app, Vite theme workbench, and the vanilla TypeScript, React SPA, Next.js App Router, Svelte, and Vue case-board examples, then ran Playwright visual smoke for the six framework apps with zero failures. The generated local review material is under `reports/testing-session/` and `reports/example-visual-smoke/`. Local testing URLs from the latest run are `http://127.0.0.1:4174/`, `http://127.0.0.1:4175/`, `http://127.0.0.1:4173/`, and `http://127.0.0.1:4180/` through `http://127.0.0.1:4184/`.
