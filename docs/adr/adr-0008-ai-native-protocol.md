...
- Extended CEM `_ashlar` namespace is an Ashlar-specific extension; consumers expecting vanilla CEM see additional fields.
- MCP server must be operated reliably (or run locally per consumer).
- AGENTS.md content must stay in sync with the codebase as Ashlar evolves.
