...
**Tailwind is not the component authoring layer.** markup primitive CSS is written in semantic class names referencing CSS custom properties (`.ashlar-button { background: var(--ashlar-color-action-primary-bg); … }`), not Tailwind utility classes hard-coded into component source.
