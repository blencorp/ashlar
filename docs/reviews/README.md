...
`pnpm ashlar release review-record-check` validates these records before readiness. For stable-evidence records, it reruns `evidence review-status` against the recorded bundle path and verifies local publication receipts from `pnpm ashlar evidence publish --output` against the signed registry state, so a completed-looking markdown file cannot count while the underlying reviewer bundle is still blocked or unpublished. For release-trust records, local registry, SBOM, attestation, and trust-bundle paths are reverified with `pnpm ashlar release verify-trust-bundle`; local npm provenance paths must be JSON output from `pnpm ashlar release provenance-verify-public --json` that matches the reviewed package versions; local Capsule Sigstore verification paths must be JSON output from `pnpm ashlar release public-trust-verify --json` that matches the reviewed signed registry artifact. The Sigstore release artifact must include the complete signed `registry/` directory, not only generated capsule manifests and bundle files, so a reviewer can rerun public-trust verification from the downloaded artifact. HTTPS artifact links are treated as external reviewer evidence. For design-partner records, local screen, validator-output, and reviewer-checklist paths must exist.

`pnpm ashlar release readiness` fails strict replacement-grade readiness until all three top-level record types exist and pass the record checker.
